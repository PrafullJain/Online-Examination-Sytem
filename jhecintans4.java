...
public class jhecintans4 extends JFrame implements WindowListener{
	...
	public jhecintans4() {
		...
		l8.setBounds(193,276,560,70);
		l9.setBounds(56,294,360,120);
		l10.setBounds(164,309,1200,90);
	...
	try
		{
			...
			String qa="select * from jint where sno=7";
			OracleResultSet rs1=(OracleResultSet) stmt.executeQuery(qa);
			while(rs1.next())
			{
				...
				String s1=rs1.getString("EXPLANATION");
				...
				for(int i=0;i<len;i++)
				{
					sb[i]=new StringBuffer(strarr[i]);
					if(i==len1)
					{
						sb[i]=sb[i].append(System.getProperty("line.separator"));
					}
				}
				String[] strarr2= new String[sb.length];
				char[] c1=new char[sb.length];
				StringBuffer fstbf =new StringBuffer();
				for(int i=0;i<sb.length;i++)
				{
					strarr2[i]=sb[i].toString();
					c1=strarr2[i].toCharArray();
					fstbf.append(Arrays.toString(c1).replace(",", "").replace("[", "")
							.replace("]", ""));
				}
				String fstr=fstbf.toString();
				System.out.println(fstr);
				l10.setText(fstr);
			}
			rs1.close();
			String q2="select * from jint where sno=8";
...
public static void main(String[] args) {
	jhecintans4 c1=new jhecintans4();
...
public void windowClosing(WindowEvent e) {
	...
	final String msg="<html><big>Thank you for using <u>Online-Examination System</u></big><hr><hr>"
